import React, {useEffect} from 'react';
import bem from "easy-bem";
import {Carousel} from '3d-react-carousal';
import { Player, ControlBar, VolumeMenuButton, playerActions} from 'video-react';
import {useDispatch, useSelector} from "react-redux";
import "react-multi-carousel/lib/styles.css";
import {fetchCarousel, fetchMove} from "../../redux/carouselSlice";
import CarouselImages from "../../component/CarouselImages/CarouselImages";

let mas = []
const Rek = () => {
    const b = bem("scroll");
    const statePlay = useSelector(state => state.carousel.cinemaRek);
    const dispatch = useDispatch();
    const stateImagesTop = useSelector(state => state.carousel.moveCinema);

    useEffect(() => {
        dispatch(fetchCarousel())
    }, [stateImagesTop])

    useEffect(() => {
        dispatch(fetchMove())
    }, [])


    function removeDuplicates(originalArray, prop) {
        let newArray = [];
        let lookupObject  = {};

        for(let i in originalArray) {
            lookupObject[originalArray[i][prop]] = originalArray[i];
        }

        for(let i in lookupObject) {
            newArray.push(lookupObject[i]);
        }
        return newArray;
    }

    let slides = statePlay.map(item => {
        return <>
            <Player
                ref={play => {

                    if (play){
                        let obj = play;
                        obj.id = item.id;
                        mas.push(obj);
                    }

                }}
                poster={item.poster_url}
                src={item.trailer_url}
                key={item.id}
            >
                <ControlBar autoHide={false} disableDefaultControls>
                    <VolumeMenuButton />
                    <VolumeMenuButton vertical />
                </ControlBar>
            </Player>
            <div className="textWhite">
                <h2 >{item.title}</h2> </div>
            <div id="soundYes" onClick={() => soundYes(item.id)} className="textSound d-block">
                <svg xmlns="http://www.w3.org/2000/svg" width="42" height="42" fill="currentColor"
                     className="bi bi-volume-up" viewBox="0 0 16 16">
                    <path
                        d="M11.536 14.01A8.473 8.473 0 0 0 14.026 8a8.473 8.473 0 0 0-2.49-6.01l-.708.707A7.476 7.476 0 0 1 13.025 8c0 2.071-.84 3.946-2.197 5.303l.708.707z"/>
                    <path
                        d="M10.121 12.596A6.48 6.48 0 0 0 12.025 8a6.48 6.48 0 0 0-1.904-4.596l-.707.707A5.483 5.483 0 0 1 11.025 8a5.483 5.483 0 0 1-1.61 3.89l.706.706z"/>
                    <path
                        d="M10.025 8a4.486 4.486 0 0 1-1.318 3.182L8 10.475A3.489 3.489 0 0 0 9.025 8c0-.966-.392-1.841-1.025-2.475l.707-.707A4.486 4.486 0 0 1 10.025 8zM7 4a.5.5 0 0 0-.812-.39L3.825 5.5H1.5A.5.5 0 0 0 1 6v4a.5.5 0 0 0 .5.5h2.325l2.363 1.89A.5.5 0 0 0 7 12V4zM4.312 6.39 6 5.04v5.92L4.312 9.61A.5.5 0 0 0 4 9.5H2v-3h2a.5.5 0 0 0 .312-.11z"/>
                </svg>
            </div>

            <div id="soundNot" onClick={() => soundNot(item.id)} className="textSound d-none">
                <svg xmlns="http://www.w3.org/2000/svg" width="42" height="42" fill="currentColor"
                     className="bi bi-volume-mute" viewBox="0 0 16 16">
                    <path
                        d="M6.717 3.55A.5.5 0 0 1 7 4v8a.5.5 0 0 1-.812.39L3.825 10.5H1.5A.5.5 0 0 1 1 10V6a.5.5 0 0 1 .5-.5h2.325l2.363-1.89a.5.5 0 0 1 .529-.06zM6 5.04 4.312 6.39A.5.5 0 0 1 4 6.5H2v3h2a.5.5 0 0 1 .312.11L6 10.96V5.04zm7.854.606a.5.5 0 0 1 0 .708L12.207 8l1.647 1.646a.5.5 0 0 1-.708.708L11.5 8.707l-1.646 1.647a.5.5 0 0 1-.708-.708L10.793 8 9.146 6.354a.5.5 0 1 1 .708-.708L11.5 7.293l1.646-1.647a.5.5 0 0 1 .708 0z"/>
                </svg>
            </div>

        </>
    });

    const callback = (i) => {
        let contentDuplicates = removeDuplicates(mas, "id");
        let block = document.querySelectorAll("#block");
        const sound = document.querySelectorAll("#sound");

        if (mas.length) {
            contentDuplicates.map(item => {
                item.load();
            })
        }
        block.forEach(item => {
            item.classList.remove("d-none");
            item.classList.add("d-block")
        })

        sound.forEach(item => {
            item.classList.remove("d-block");
            item.classList.add("d-none");
        })
    }

    const soundYes = () => {
        let contentDuplicates = removeDuplicates(mas, "id");
        const soundPlay = document.querySelectorAll("#soundYes");
        const soundNot = document.querySelectorAll("#soundNot");

        soundPlay.forEach(item => {
            item.classList.remove("d-block")
            item.classList.add("d-none")
        })

        soundNot.forEach(item => {
            item.classList.remove("d-none")
            item.classList.add("d-block")
        })

        contentDuplicates.forEach(item => {
            item.muted = true
        })

    }

    const soundNot = () => {
        let contentDuplicates = removeDuplicates(mas, "id");
        const soundPlay = document.querySelectorAll("#soundYes");
        const soundNot = document.querySelectorAll("#soundNot");

        soundNot.forEach(item => {
            item.classList.remove("d-block")
            item.classList.add("d-none")
        })

        soundPlay.forEach(item => {
            item.classList.remove("d-none");
            item.classList.add("d-block")
        })

        contentDuplicates.forEach(item => {
            item.muted = false
        })
    }


    return (
        <div className={b()}>
        <div  className={b("fons")}>
            {statePlay.length ?
                <Carousel slides={slides} interval={1} onSlideChange={callback}>
                </Carousel> : <div className="d-flex justify-content-center">
                    <div className="spinner-border text-light  mt-5" role="status">
                        <span className="visually-hidden">Loading...</span>
                    </div>
                </div>
            }
        </div>
            <CarouselImages title="Топ новинки" data={stateImagesTop}/>
        </div>
    );
};


const RekItem = () => {

    return (
    <div>
        <video width="320" height="240" controls={false}>
            <source src="movie.mp4" type="video/mp4"/>
            <source src="movie.ogg" type="video/ogg"/>
        </video>
    </div>
    );
};


export default Rek;

